...
func (t *MilkChain) Init(APIstub shim.ChaincodeStubInterface) pb.Response {
...
func (t *MilkChain) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
...
func (t *MilkChain) GetTxTimestampChannel(APIstub shim.ChaincodeStubInterface) (string, error) {
...
func (t *MilkChain) initLedger(APIstub shim.ChaincodeStubInterface, args []string) pb.Response {
	// seed admin
	entityUser := User{Name: "admin", User_ID: "admin", Email: "admin@pg.com", User_Type: "admin", Address: "VietNam", Password: "adminpw"}
...
// sign in
func (t *MilkChain) signIn(APIstub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) != 3 {
		return shim.Error("Incorrect number of arguments. Expected 3 argument")
	}

	// type - username - password
	assetType := args[0]
	if assetType == "admin" {
		adminBytes, err := APIstub.GetState(args[1])
		if err != nil {
		...
		if admin.Password != args[2] {
			return shim.Error("Either id or password is wrong")
		}
		return shim.Success(adminBytes)
	} else {
...
// create user
func (t *MilkChain) createUser(APIstub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	var comAsset = User{Name: args[0], User_ID: "User" + strconv.Itoa(userCounter), Email: args[1],
		User_Type: args[2], Address: args[3], Password: args[4]}
...
func (t *MilkChain) createProduct(APIstub shim.ChaincodeStubInterface, args []string) pb.Response {

	//To check number of arguments are 3
	if len(args) != 5 {
		return shim.Error("Incorrect number of arguments, Required 5 arguments" + string(len(args)))
	}
	...
	if len(args[2]) == 0 {
		return shim.Error("Manufacturer Name must be provided")
	}
	...
	if len(args[4]) == 0 {
		return shim.Error("Description must be non-empty ")
	}
...
func (t *MilkChain) updateProduct(APIstub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	product.Description = args[4] // product desc from UI for the update
...
func (t *MilkChain) orderProduct(APIstub shim.ChaincodeStubInterface, args []string) pb.Response {
	// parameter length check
	if len(args) != 2 {
		return shim.Error("Incorrect number of arguments, Required 4")
	}
...
func (t *MilkChain) deliveredProduct(APIstub shim.ChaincodeStubInterface, args []string) pb.Response {
	// parameter length check
	if len(args) != 1 {
		return shim.Error("Incorrect number of arguments, Required 4")
	}
...
func (t *MilkChain) sellToConsumer(APIstub shim.ChaincodeStubInterface, args []string) pb.Response {
...
// queryAsset
func (t *MilkChain) queryAsset(APIstub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) != 1 {
...
func (t *MilkChain) queryAll(APIstub shim.ChaincodeStubInterface, args []string) pb.Response {
...
func (t *MilkChain) getHistory(APIstub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	productId := args[0]
	resultsIterator, err := APIstub.GetHistoryForKey(productId)
	if err != nil {
		return shim.Error(err.Error())
	}
	defer resultsIterator.Close()
